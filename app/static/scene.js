...
const loadTexture = file => {
    if(!main.textureLoader)
        main.textureLoader = new THREE.TextureLoader().setPath( 'backgrounds/' );
    
    main.textureLoader.load( file, function ( texture ) {  // handle img backgrounds
        main.scene.background = texture;
        texture.dispose();
    } );
};
...
export const setBackground = background => {
    if(main.scene.background === background)  // continue if same background
        return;
    ...
    try {
        ...
        if(ext === 'hdr' || ext === 'hdri')
            loadHdr(background);        // Either a HDR file
        else
            loadTexture(background);    // Or a simple image
    }
